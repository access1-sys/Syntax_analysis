...
#from other_sem_CNN1_LSTM import Overall_model_CRF
...
def train(epoch,dataloader):
    ...
    with tqdm(dataloader) as t:
        for text,text_glove,pos,adj,bio_label in t:
            ...
            MWE_model.zero_grad()
            loss=MWE_model.neg_log_likelihood(text_glove,pos,adj,targets)
    ...
    # print("已完成：{} 轮训练\t准确率为：{}/{} ({:.2f}%)\t平均损失为：({:.2f})\n".format(epoch,correct,len(dataloader),
    #                                                                  100.*correct/len(dataloader),
    #                                                                  1.*average_loss/len(dataloader)))
...
if __name__=='__main__':
    ...
    model_name='7_after_LSTM_CNN_1'       #保存的模型名字

    #MWE_model.load_state_dict(torch.load(os.path.join(config.model3_path, 'Overall_model_CRF_1.pth')))
    ...
    for i in range(50):
        ...
        if test_max<=test_precision:
            test_max=test_precision
