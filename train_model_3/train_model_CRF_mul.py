...
from train_model_mul import Overall_modedl
...
class Overall_model_CRF(nn.Module):
    ...
    def _get_lstm_features(self,text_glove,pos,adj_sd,adj_pmi):
        '''
        # 包含 词嵌入层+LSTM+线形层，
        :param sentence: 句子
        :return: 特征，[len(sentence),self.tagset_size]
        '''
        output=self.overall_model(text_glove,pos,adj_sd,adj_pmi)
    ...
    def neg_log_likelihood(self,text_glove,pos,adj_sd,adj_pmi,tags):
        feats = self._get_lstm_features(text_glove,pos,adj_sd,adj_pmi)
        forward_score = self._forward_alg(feats)
        gold_score = self._score_sentence(feats, tags)
        return forward_score - gold_score

    def forward(self,text_glove,pos,adj_sd,adj_pmi):  # 不要将此与上述所有内容混淆。
        # 从 BiLSTM 获取排放分数
        lstm_feats = self._get_lstm_features(text_glove,pos,adj_sd,adj_pmi)
...
#
# model=Overall_model_CRF(tag_to_ix)
...
#     output=model(text_glove,pos,adj_sd,adj_pmi)
#     print(output)
#     break
