...
class Paper50Dataset(Dataset):
    def __init__(self,train):
        ...
        if train:
            self.mode_data=data[:int(data.shape[0]*0.9)]
        else:
            self.mode_data=data[int(data.shape[0]*0.9):]
    ...
    def __getitem__(self, item):
        ...
        text_glove=self.glove_dict.transform(text.split(),max_len=60)
        pos=self.ws_dict.transform(pos.split(),max_len=60)
        #adj=self.adjacency(str(text),max_len=len(text.split()))
        #adj=self.PMI_adj(text.split(),max_len=len(text.split()))
        adj=[1]
        add=''
        O=''
        for i in range(60-len(text.split())):
            add+=' PAD'
            O+=' O'
        text+=add
        bio_label+=O
        ...
        return text.strip(),text_glove,pos,adj,bio_label
